...
library(pegas)
library(vcfR)
setwd("~/Dropbox/afrodyn_data/clustering/mona/")
...
# read in VCF
###
vcf <- read.vcfR("fries_filtered_maf.vcf",checkFile = T, convertNA = T) #read in all data
vcf 
...
#save.image("~/Dropbox/afrodyn_data/stable_areas/data/mona.Rdata")
...
#check match with stable_areas
sort(table(dapc1$grp))
df<-read.csv("../../stable_areas/locs/mona.csv")
...
genotype<-read.lfmm("fries_filtered_maf.lfmm")
...
#png("~/Dropbox/projects/AJH_AFRODYN/AFRODYN_shared/afrodyn_paper/review2_files/supp_figures/mona_xval.png",height=750,width=750)
...
pdf("~/Dropbox/projects/AJH_AFRODYN/AFRODYN_shared/afrodyn_paper/review2_files/supp_figures/tess/mona_barplot.pdf",height=4,width=10)
...
library(raster)
library(rworldmap)
png("~/Dropbox/projects/AJH_AFRODYN/AFRODYN_shared/afrodyn_paper/review2_files/supp_figures/tess/mona_tess.png",height=750,width=750)
