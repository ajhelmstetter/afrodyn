...
library(pegas)
library(vcfR)
setwd("~/Dropbox/afrodyn_data/clustering/podo_a/")
...
# read in VCF
###
vcf <- read.vcfR("podo_a_filtered_maf.vcf",checkFile = T, convertNA = T) #read in all data
vcf 
...
#save.image("~/Dropbox/afrodyn_data/stable_areas/data/podo_a.Rdata")
...
#remove inds without coords
genotype<-genotype[1:34,]
coordinates<-coordinates[1:34,]
...
#png("~/Dropbox/projects/AJH_AFRODYN/AFRODYN_shared/afrodyn_paper/review2_files/supp_figures/podo_a_xval.png",height=750,width=750)
...
axis(1, at = 1:nrow(q.matrix), labels = foo$voucher[1:34], las = 3, cex.axis = .4) 
#dev.off()
