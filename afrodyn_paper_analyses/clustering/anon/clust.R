...
library(pegas)
library(vcfR)
setwd("~/Dropbox/afrodyn_data/clustering/anon/")
...
#save.image("~/Dropbox/afrodyn_data/stable_areas/data/anon.Rdata")
...
df<-read.csv("../../stable_areas/locs/anon.csv")

sort(table(df$cluster))

data.frame(dapc1$grp)
...
#png("~/Dropbox/projects/AJH_AFRODYN/AFRODYN_shared/afrodyn_paper/review2_files/supp_figures/anon_xval.png",height=750,width=750)
plot(tess3.obj, pch = 19, col = "blue",
     xlab = "Number of ancestral populations",
     ylab = "Cross-validation score")
#dev.off()

# retrieve tess3 Q matrix for K = 7 clusters 
q.matrix <- qmatrix(tess3.obj, K = 7)
my.palette <- CreatePalette(color.vector = c(brewer.pal(7,"Set2")))

# STRUCTURE-like barplot for the Q-matrix
pdf("~/Dropbox/projects/AJH_AFRODYN/AFRODYN_shared/afrodyn_paper/review2_files/supp_figures/tess/anon_barplot.pdf",height=4,width=10)
...
library(raster)
library(rworldmap)
png("~/Dropbox/projects/AJH_AFRODYN/AFRODYN_shared/afrodyn_paper/review2_files/supp_figures/tess/anon_tess.png",height=750,width=750)
